...
struct rectangle_tap {
  int x;
  int y;
  int width;
  int height;
};
typedef struct rectangle_tap rectangle_t;
...
rectangle_t intersection(rectangle_t r1, rectangle_t r2) {
  ...
  if ((max(r1.x, r2.x) > min((r1.x + r1.width), (r2.x + r2.width))) ||
    ...
    tem.x = max(r1.x, r2.x);
    tem.y = max(r1.y, r2.y);
    tem.width = min(r1.x + r1.width, r2.x + r2.width) - tem.x;
    tem.height = min(r1.y + r1.height, r2.y + r2.height) - tem.y;
    r1 = tem;
  }
  return r1;
}
