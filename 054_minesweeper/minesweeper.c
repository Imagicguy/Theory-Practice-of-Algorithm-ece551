...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  int ** pointerArr = malloc(h * sizeof(*pointerArr));
  for (int i = 0; i < h; i++) {
    pointerArr[i] = malloc(w * sizeof(*pointerArr[i]));
    for (int j = 0; j < w; j++) {
      pointerArr[i][j] = UNKNOWN;
    }
    new_board->board = pointerArr;
  }
...
int countMines(board_t * b, int x, int y) {
  int counts = 0;
  int direction[8][2] = {{-1, -1}, {1, 1}, {-1, 0}, {1, 0}, {0, 1}, {0, -1}, {1, -1}, {-1, 1}};
  for (int i = 0; i < 8; i++) {
    if (x + direction[i][0] < b->width && x + direction[i][0] >= 0 && y + direction[i][1] >= 0 &&
        y + direction[i][1] < b->height) {
      if (IS_MINE(b->board[y + direction[i][1]][x + direction[i][0]])) {
        counts++;
      }
    }
  }
