int isBetween(int x,int y,int x_offset,int y_offset,int size2){
...
int isLess(int x,int y,int size1){
...
void squares(int size1, int x_offset, int y_offset, int size2) {
  ...
  //	
   //count from 0 to h. Call the number you count with y
	for (int y = 0;y < h;y++){
		for (int x = 0;x < w;x++){
			if(isBetween(x,y,x_offset,y_offset,size2)){
				printf("*");
			}
			else if(isLess(x,y,size1)){
				printf("#");
			}
			else{
				printf(" ");
